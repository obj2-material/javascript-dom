    ...
    render() {
        return (
            <div className="container" style={{marginLeft: "20px", marginRight: "20px"}}>
                <DatosDelAvionElegido mainComponent={this} />
                <PanelSeleccionAvion mainComponent={this} />
                { this.tablaVuelos() }
            </div>
        )
    }
...
class DatosDelAvionElegido extends React.Component {
    mainComponent() { return this.props.mainComponent }
    avionElegido() { return this.mainComponent().avionElegido() }
...
class PanelSeleccionAvion extends React.Component {
    mainComponent() { return this.props.mainComponent }
    avionElegido() { return this.mainComponent().avionElegido() }
    elegirAvion(nombreAvion) { this.mainComponent().elegirAvion(nombreAvion) }

    render() {
        return this.panelSeleccionAvionDinamico()
    ...
    panelSeleccionAvionDinamico() {
        return (
            <div className="panel panel-info">
                <div className="panel-body">
                    <div className="text-center">
                        {ventas.store.aviones().map(avion => this.seleccionAvion(avion.nombre()))}
                    </div>
                </div>
            </div>
        )
